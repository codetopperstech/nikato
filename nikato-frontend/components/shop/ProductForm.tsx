...
export function ProductForm({ defaultValues, categories, onSubmit, isLoading }: ProductFormProps) {
  ...
  const inputCls = (hasError: boolean) =>
    `w-full border-[1.5px] rounded-xl px-4 py-3 text-sm outline-none transition-all bg-white ${hasError ? 'border-red-400 bg-red-50/50' : 'border-gray-200 focus:border-brand focus:ring-2 focus:ring-brand/15'}`;

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-1.5">Product Name *</label>
        <input {...register('name')} placeholder="e.g. Fresh Tomatoes" className={inputCls(!!errors.name)} />
        {errors.name && <p className="text-red-500 text-xs mt-1">{errors.name.message}</p>}
      </div>

      <div>
        <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-1.5">Description</label>
        <textarea {...register('description')} rows={2} placeholder="Short description..." className={`${inputCls(false)} resize-none`} />
      </div>

      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-1.5">Selling Price (₹) *</label>
          <input type="number" step="0.01" {...register('price')} placeholder="0.00" className={inputCls(!!errors.price)} />
          {errors.price && <p className="text-red-500 text-xs mt-1">{errors.price.message}</p>}
        </div>
        <div>
          <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-1.5">MRP (₹)</label>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-1.5">Stock *</label>
        ...
        <div>
          ...
          <select {...register('unit')} className={inputCls(!!errors.unit)}>
            {['piece','kg','g','l','ml','dozen','pack','box','bundle'].map(u => <option key={u} value={u}>{u}</option>)}
          </select>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        <label className="flex-1 flex items-center justify-between bg-white border-[1.5px] border-gray-200 rounded-xl px-4 py-3 cursor-pointer hover:border-gray-300 transition-colors">
          <span className="text-sm font-semibold text-gray-700">Veg item</span>
          <div className="relative">
            <input type="checkbox" {...register('is_veg')} className="sr-only peer" />
      ...
      <button type="submit" disabled={isLoading}
        ...
        {isLoading ? <div className="w-5 h-5 rounded-full border-2 border-white border-t-transparent animate-spin" /> : defaultValues?.name ? 'Save Changes' : 'Add Product'}
      </button>
    </form>
  );
}
