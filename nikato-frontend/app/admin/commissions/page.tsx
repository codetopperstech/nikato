...
type ShopRow = Shop & { owner: { full_name: string } };
...
export default function AdminCommissionsPage() {
  ...
  async function saveCommission(id: string) {
    ...
    if (isNaN(rate) || rate < 0 || rate > 1) {
      toast.error('Invalid rate', 'Enter a value between 0 and 1 (e.g. 0.10 for 10%)');
      return;
    }
    setSaving(true);
          ...
          actions={(shop) =>
            editingId === shop.id ? (
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => { setEditingId(shop.id); setEditRate(shop.commission_rate.toString()); }}
              >
                Edit
              </Button>
            )
