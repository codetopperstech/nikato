...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const item of cart as {product_id: string; quantity: number}[]) {
      const { error: stockErr } = await admin.rpc('decrement_stock', { p_product_id: item.product_id, p_quantity: item.quantity });
      if (stockErr) {
        // Rollback already-decremented items before returning
        await Promise.all(decremented.map(d => admin.rpc('restore_stock', { p_product_id: d.product_id, p_quantity: d.quantity })));
        const p = products.find(p => p.id === item.product_id)!;
        return NextResponse.json({ error: `"${p.name}" is out of stock` }, { status: 422 });
      }
    ...
    // COD: stays 'pending' — shop must confirm before a rider is assigned
