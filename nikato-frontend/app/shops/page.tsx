...
export default function ShopsPage() {
  ...
  const [radiusKm, setRadiusKm] = useState(3); // ✅ default 3km
  ...
  if (!hasLocation) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#FAFAF8] px-6">
        ...
        <p className="text-sm text-gray-500 text-center mb-6 max-w-xs">We need your location to show shops near you within 3 km.</p>
        <Button variant="primary" size="lg" onClick={requestGPS} isLoading={permissionState === 'requesting'}>Allow location access</Button>
        {permissionState === 'denied' && <p className="text-xs text-red-500 mt-3 text-center">Location denied. Please enable it in browser settings and refresh.</p>}
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FAFAF8]">
      <div className="sticky top-0 z-20 bg-white border-b border-gray-100 px-4 py-3 space-y-3">
        <div className="flex items-center gap-2">
          <div className="flex-1">
            <Input placeholder="Search shops…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} leftAddon={<Search size={16} />} className="py-2" />
          </div>
          <div className="flex bg-gray-100 rounded-xl p-1 gap-1">
            <button onClick={() => setViewMode('list')} className={cn('p-2 rounded-lg transition-all', viewMode === 'list' ? 'bg-white shadow-sm text-[#FF6B35]' : 'text-gray-500')}><List size={16} /></button>
            <button onClick={() => setViewMode('map')} className={cn('p-2 rounded-lg transition-all', viewMode === 'map' ? 'bg-white shadow-sm text-[#FF6B35]' : 'text-gray-500')}><Map size={16} /></button>
          </div>
        </div>
        <div className="flex items-center gap-2 text-xs text-gray-500">
          ...
          <span>Radius:</span>
          {[1, 3, 5].map(r => (
            <button key={r} onClick={() => setRadiusKm(r)} className={cn('px-2.5 py-1 rounded-full border text-xs font-medium transition-all', radiusKm === r ? 'bg-[#FF6B35] border-[#FF6B35] text-white' : 'border-gray-200 text-gray-600 hover:border-gray-400')}>
              {r} km
            </button>
          ))}
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-4">
        {isLoading ? (
          ...
          <EmptyState icon={<span className="text-5xl">🏪</span>} title="No shops found" description={searchQuery ? `No results for "${searchQuery}"` : `No shops within ${radiusKm} km. Try increasing radius.`} action={!searchQuery ? <Button variant="ghost" size="sm" onClick={() => setRadiusKm(5)}>Try 5 km</Button> : <Button variant="ghost" size="sm" onClick={() => setSearchQuery('')}>Clear search</Button>} />
        ) : (
          <>
            {openShops.length > 0 && (
              <section>
