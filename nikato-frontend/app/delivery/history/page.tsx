...
export default function DeliveryHistoryPage() {
  ...
  const { data: orders = [], isLoading } = useQuery<Order[]>({
    ...
    queryFn: async () => {
      const { data } = await supabase.from('orders').select('*, shop:shops(name)')
        .eq('delivery_partner_id', user!.id).in('status', ['delivered', 'cancelled'])
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      {isLoading ? (
        ...
        <EmptyState icon={<Clock size={44} className="text-gray-200" />} title="No deliveries yet" description="Your completed deliveries will appear here" />
      ) : (
            ...
            return (
              <div key={o.id} className="bg-white rounded-2xl border border-gray-100 p-4 flex items-center gap-3">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500 truncate">{shop?.name} · {formatOrderDate(o.created_at)}</p>
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-sm font-bold text-green-600">+{formatPrice(o.delivery_earning)}</p>
                  <Badge variant={o.status === 'delivered' ? 'success' : 'danger'}>{o.status}</Badge>
                </div>
              </div>
            );
          })}
