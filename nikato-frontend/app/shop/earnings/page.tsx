...
export default function ShopEarningsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const rows = (orders ?? []) as EarningRow[];
      const sum = (from: string) =>
        rows.filter((r) => r.created_at >= from).reduce((acc, r) => acc + (r.shop_earning ?? 0), 0);
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="bg-white rounded-2xl border border-gray-100">
            ...
            {data.rows.length === 0 ? (
              <EmptyState
                icon={<Wallet size={40} className="text-gray-200" />}
                  ...
                  <div key={row.id} className="flex items-center justify-between px-4 py-3">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-400">
                        Order: {formatPrice(row.total_amount)} · Commission: {formatPrice(row.commission_amount)}
                      </p>
                    </div>
                  </div>
                ))}
