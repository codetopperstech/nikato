    ...
    <div className="p-4 lg:p-6 max-w-3xl">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-4">
        <button
          ...
          className={`flex-shrink-0 px-3 py-1.5 rounded-xl text-sm font-medium transition-colors ${filterCat === 'all' ? 'bg-[#FF6B35] text-white' : 'bg-gray-100 text-gray-600'}`}
        >
          All
        </button>
        {categories.map((c) => (
          <button
            ...
            className={`flex-shrink-0 px-3 py-1.5 rounded-xl text-sm font-medium transition-colors ${filterCat === c.id ? 'bg-[#FF6B35] text-white' : 'bg-gray-100 text-gray-600'}`}
          >
            {c.name}
          </button>
        ))}
      ...
      {isLoading ? (
        ...
        <EmptyState title="No products yet" description="Add your first product to start selling" action={<Link href="/shop/products/new"><Button>Add Product</Button></Link>} />
      ) : (
