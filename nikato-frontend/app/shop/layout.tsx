...
export default function ShopLayout({ children }: { children: React.ReactNode }) {
  ...
  const fetchPending = useCallback(async (shopId: string) => {
    const { data } = await supabase.from('orders').select('*').eq('shop_id', shopId).eq('status', 'pending').order('created_at', { ascending: false });
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`shop-orders-${shopId}`)
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'orders', filter: `shop_id=eq.${shopId}` }, () => {
        qc.invalidateQueries({ queryKey: ['shop-orders', shopId] }); fetchPending(shopId);
      })
      .subscribe((status) => { if (status === 'CHANNEL_ERROR') setTimeout(() => channel.subscribe(), 3000); });
  ...
  useEffect(() => {
    ...
    const shopChannel = supabase.channel(`shop-state-${shopId}`)
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'shops', filter: `id=eq.${shopId}` }, (payload) => {
        setIsOpen((payload.new as Shop).is_open);
        qc.invalidateQueries({ queryKey: ['my-shop', user?.id] });
      }).subscribe();
    shopChannelRef.current = shopChannel;
    return () => { if (shopChannelRef.current) { supabase.removeChannel(shopChannelRef.current); shopChannelRef.current = null; } };
  }, [shopData?.id, user?.id, setIsOpen, qc]);

  useEffect(() => {
    if (!shopData?.id) return;
    const shopId = shopData.id;
    const onVisibilityChange = () => { if (!document.hidden) { qc.invalidateQueries({ queryKey: ['pending-orders', shopId] }); } };
    const onOnline = () => { if (!isOnlineRef.current) { isOnlineRef.current = true; toast.success('Back online', 'Syncing…'); qc.invalidateQueries({ queryKey: ['pending-orders', shopId] }); } };
    const onOffline = () => { isOnlineRef.current = false; toast.error('You are offline'); };
    document.addEventListener('visibilitychange', onVisibilityChange);
    window.addEventListener('online', onOnline);
    window.addEventListener('offline', onOffline);
    return () => { document.removeEventListener('visibilitychange', onVisibilityChange); window.removeEventListener('online', onOnline); window.removeEventListener('offline', onOffline); };
  }, [shopData?.id, qc]);
