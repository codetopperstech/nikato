  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/services/book', {
        ...
        body: JSON.stringify({ name, phone, address, service_type: service, scheduled_date: date, scheduled_time: time, notes }),
      });
  ...
  if (success) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6" style={{ background: '#F9FBF8' }}>
      ...
      <p className="text-xs text-gray-400 text-center mb-8">
        Please keep your phone reachable on <span className="font-bold text-gray-600">{phone}</span>.
      </p>
      <Link href="/"
          ...
          <div className="grid grid-cols-3 gap-2">
            {SERVICES.map(({ label, slug, emoji }) => (
              <button key={slug} type="button" onClick={() => setService(slug)}
                className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border-[1.5px] transition-all active:scale-95 ${
                  service === slug
                    ? 'border-[#7ED957] bg-[#7ED95712] shadow-sm'
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div>
            ...
            <textarea required value={address} onChange={e => setAddress(e.target.value)}
              placeholder="Full address where service is needed" rows={2}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input required type="date" value={date} min={today}
                onChange={e => setDate(e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Time</label>
              <input required type="time" value={time}
          ...
          <div>
            ...
            <textarea value={notes} onChange={e => setNotes(e.target.value)}
              placeholder="Describe the issue or any special requirements..." rows={3}
              className={inputCls + ' resize-none'} />
          </div>
